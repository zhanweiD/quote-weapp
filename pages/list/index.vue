	...
	<view class="page">
		...
		<view class="cate-content">
			...
			<view class="content-right">
				...
				<scroller v-if="selectBrandId" @init="initScroller" @down="refreshData" @up="getData" :up="optUp" @scroll="navFloatShow(scroller)" :fixed="false">
	...
	export default {
		...
		data() {
			return {
				...
				scroller: {},
				optUp: { auto: true, onScroll: true, page: { size: 20 }, empty: { tip: '暂无数据~' } },
				category_index: 0,
				showNavFloat: false,
			}
		},
		...
		methods: {
			...
			getData() {
				...
				this.$app.request({
					...
					data: {
						page: this.scroller.num || 1,
						row: this.scroller.size || 20,
						type: '1',
					...
					success: res => {
						if (res.code == 1) {
							...
							this.scroller.endByPage(res.data.length, this.scroller.num);
						} else {
